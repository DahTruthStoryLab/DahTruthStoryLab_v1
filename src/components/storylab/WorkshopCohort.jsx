...
const CohortBanner = ({ cohort, onEdit }) => {
  ...
  const avgAttendance =
    cohort.sessions.reduce((acc, s) => {
      ...
      return (
        acc + s.attendance.filter((a) => a.attended).length / s.attendance.length
      );
    }, 0) /
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center">
        ...
        <h3 className="font-semibold text-ink text-center mb-1">
          {participant.name}
        </h3>
        {participant.genre && (
...
const SessionCard = ({
  ...
  return (
    <div className={`${glassLight} rounded-xl p-4 transition hover:shadow`}>
      <button onClick={onToggle} className="w-full text-left">
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-col items-end gap-2">
            <span
              className={`text-xs px-2 py-1 rounded-full border ${statusColors[session.status]}`}
            >
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-top border-t border-white/40 space-y-3">
          <div>
...
export default function WorkshopCohort() {
  ...
  const updateSession = () => {
    // Placeholder: you can replace with a real modal later
  ...
  const sendMessage = (text) => {
    ...
    setCohort({
      ...cohort,
      messages: [...cohort.messages, newMessage],
    });
  };
