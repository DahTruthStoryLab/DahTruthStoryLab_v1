...
// All paths verified against FictionRoutes, NonfictionRoutes, PoetryRoutes
...
// ─────────────────────────────────────────────
//  STUDIO LABEL
//  Optional navPath makes the label clickable
// ─────────────────────────────────────────────
  ...
  return (
    <div className="px-3 pt-4 pb-1">
      ...
      {navPath ? (
        ...
        >
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] group-hover:text-violet-500 transition-colors"
...
//  COLLAPSIBLE GROUP
// ─────────────────────────────────────────────
function NavGroup({ icon: Icon, label, basePath, topPath, items, pathname }) {
  const navigate = useNavigate();
  ...
  const handleLabelClick = (e) => {
    e.stopPropagation();
    navigate(topPath || basePath);
  };

  return (
    <>
      <div className={`w-full flex items-center rounded-xl transition-all duration-150
        ${groupActive && !open ? "bg-white/90 shadow-sm" : "hover:bg-white/70"}`}>
        {/* Label — navigates to top-level page */}
        <button
          type="button"
...
export default function AppSidebar({ isOpen, onClose, userNovels = [] }) {
  ...
  // ── FICTION items ─────────────────────────
  // All paths verified against FictionRoutes.jsx
  // Fiction sub-tools live under /story-lab/workshop/* and /story-lab/*
  const fictionItems = [
  ...
  // ── NONFICTION items ──────────────────────
  // Paths verified against NonfictionRoutes.jsx
  // Routes: /essay /memoir /research /argument /chapter
  const nonfictionItems = [
    { icon: PencilLine,    label: "Essay Builder",    path: "/story-lab/nonfiction/essay" },
    { icon: BookOpen,      label: "Memoir Scene Map", path: "/story-lab/nonfiction/memoir" },
    { icon: FileText,      label: "Research Notes",   path: "/story-lab/nonfiction/research" },
    { icon: GraduationCap, label: "Argument & Thesis",path: "/story-lab/nonfiction/argument" },
    { icon: AlignJustify,  label: "Chapter Outliner", path: "/story-lab/nonfiction/chapter" },
  ];

  // ── POETRY items ──────────────────────────
  // Paths verified against PoetryRoutes.jsx
  // Routes: /craft /revision /voice /sequence /remix
  // AI Poetry Prompts redirects to /story-lab/prompts (no dedicated page yet)
  const poetryItems = [
    { icon: Wand2,       label: "Craft Lab",        path: "/story-lab/poetry/craft" },
    { icon: RefreshCw,   label: "Revision Lab",     path: "/story-lab/poetry/revision" },
    { icon: Fingerprint, label: "Voice & Identity", path: "/story-lab/poetry/voice" },
    { icon: BookCopy,    label: "Sequence Builder", path: "/story-lab/poetry/sequence" },
    { icon: Shuffle,     label: "Remix Lab",        path: "/story-lab/poetry/remix" },
    { icon: Sparkles,    label: "AI Poetry Prompts",path: "/story-lab/prompts" },
  ];

  // ── WORKSHOP HUB items ────────────────────
  // Export + Publish moved to Author Studio
  const hubItems = [
    ...
    <>
      ...
      >
        ...
        />
          ...
          {/* Fiction — label navigates to /story-lab/fiction, chevron expands */}
          <NavGroup
            icon={BookMarked}
            label="Fiction"
            basePath="/story-lab/fiction"
            topPath="/story-lab/fiction"
            items={fictionItems}
            pathname={pathname}
          />

          {/* Nonfiction — label navigates to /story-lab/nonfiction */}
          <NavGroup
            icon={FileText}
            label="Nonfiction"
            basePath="/story-lab/nonfiction"
            topPath="/story-lab/nonfiction"
            items={nonfictionItems}
            pathname={pathname}
          />

          {/* Poetry — label navigates to /story-lab/poetry */}
          <NavGroup
            icon={Feather}
            label="Poetry"
            basePath="/story-lab/poetry"
            topPath="/story-lab/poetry"
            items={poetryItems}
            pathname={pathname}
          />

          {/* Workshop Hub — label navigates to /story-lab/hub */}
          <NavGroup
            icon={Compass}
            label="Workshop Hub"
            basePath="/story-lab/hub"
            topPath="/story-lab/hub"
            items={hubItems}
            pathname={pathname}
          />
          ...
          <NavItem
            ...
            active={
              ...
              ["/proof", "/publishing-prep", "/cover"].includes(pathname)
            }
          />
          <NavItem
            icon={Download}
            label="Export"
            path="/export"
            active={pathname === "/export" || pathname === "/format"}
          />
          <NavItem
            icon={Settings}
            label="Profile"
