...
import { Plus, Trash2, Layout, Save, ChevronDown, ChevronUp, RefreshCw } from "lucide-react";
import { ExpandableTextArea } from "./WritingPad";
import {
  getArchitecture,
  loadArchitectureFromCloud,
  saveArchitecture,
    ...
    <div className="rounded-xl border overflow-hidden transition-all"
      ...
      <div className="flex items-center justify-between px-4 py-3 cursor-pointer"
        ...
        <div className="flex items-center gap-2">
          <button onClick={(e) => { e.stopPropagation(); if (window.confirm(`Delete Chapter ${index + 1} plan?`)) onDelete(); }}
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-2 space-y-4 border-t border-slate-100">
          <Field label="Chapter Title" value={plan.chapterTitle} onChange={(v) => onChange("chapterTitle", v)} placeholder="The Wake of Daisy" />
          <div className="grid grid-cols-2 gap-4">
            <ExpandableTextArea label="Purpose of this chapter" value={plan.purpose} onChange={(v) => onChange("purpose", v)} placeholder="What must this chapter accomplish in the story?" />
            <ExpandableTextArea label="What changes by the end?" value={plan.whatChanges} onChange={(v) => onChange("whatChanges", v)} placeholder="Something must shift — in the world or in the character." />
            <ExpandableTextArea label="Who is affected?" value={plan.whoIsAffected} onChange={(v) => onChange("whoIsAffected", v)} placeholder="Which characters carry the weight of this chapter?" />
            <ExpandableTextArea label="What is revealed?" value={plan.whatIsRevealed} onChange={(v) => onChange("whatIsRevealed", v)} placeholder="Information, truth, or character the reader learns here." />
          </div>

          <div className="rounded-xl p-4 space-y-4"
            style={{ background: `${BRAND.navy}04`, border: `1px solid ${BRAND.navy}10` }}>
            <div>
              <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: BRAND.navy }}>Transition Engine</div>
              <p className="text-[11px] text-slate-500 mb-3">
                Dostoevsky ended chapters by pulling the reader into the next moment before they could stop.
                Define how this chapter ends and what it opens.
              </p>
            </div>
            <SelectField label="How does this chapter end?" value={plan.endingType} onChange={(v) => onChange("endingType", v)} options={CHAPTER_ENDING_TYPES} />
            <ExpandableTextArea label="What pulls the reader into the next chapter?" value={plan.bridgeToNext} onChange={(v) => onChange("bridgeToNext", v)} placeholder="The last image, question, or action that makes stopping impossible." rows={3} />
            <SelectField label="Pacing of this chapter" value={plan.pacingNote} onChange={(v) => onChange("pacingNote", v)} options={PACING_OPTIONS} />
          </div>

          <ExpandableTextArea label="Notes" value={plan.notes} onChange={(v) => onChange("notes", v)} placeholder="Anything else — images, symbols, lines you want to land here." />
        </div>
      )}
    </div>
  );
...
export default function StoryArchitecture() {
  ...
  function addChapterPlan() {
    setArch((prev) => ({ ...prev, chapterPlans: [...(prev.chapterPlans || []), createEmptyChapterPlan()] }));
    setSaveStatus("idle");
  }
    ...
    <div className="min-h-screen" style={{ background: "linear-gradient(180deg, #fefdfb 0%, #f1f5f9 100%)" }}>
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="rounded-3xl p-8 mb-8 relative overflow-hidden"
          style={{ background: `linear-gradient(135deg, #1a1a2e 0%, ${BRAND.navy} 40%, #6b4c8a 100%)` }}>
          ...
          <div className="relative z-10 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-white/75 mt-1">Define how your story moves, breathes, and pulls the reader forward.</p>
            </div>
          </div>
          {cloudLoading && (
        ...
        {!projectId && (
          <div className="mb-6 px-4 py-3 rounded-xl text-sm flex items-center gap-3"
            ...
            <Layout size={16} style={{ color: BRAND.goldDark }} />
            No project is open. Open a project in <strong className="mx-1">Compose</strong> first so your architecture is linked to the correct manuscript.
          </div>
        )}
        ...
        <div className="flex justify-end mb-6">
          <button onClick={handleSave} disabled={saveStatus === "saving"}
            ...
            <Save size={15} />
            {saveStatus === "saving" ? "Saving..." : saveStatus === "saved" ? "Saved ✓" : "Save Architecture"}
          </button>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-500">Story Structure</div>
                <select value={arch.storyStructure || ""} onChange={(e) => handleChange("storyStructure", e.target.value)}
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-300 bg-white">
              ...
              <div>
                <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-500">Point of View</div>
                <select value={arch.povType || ""} onChange={(e) => handleChange("povType", e.target.value)}
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-300 bg-white">
              ...
              <div>
                ...
                <select value={arch.timelineType || ""} onChange={(e) => handleChange("timelineType", e.target.value)}
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-300 bg-white">
              ...
              <SelectField label="Overall Pacing" value={arch.overallPacing} onChange={(v) => handleChange("overallPacing", v)} options={PACING_OPTIONS} />
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <ExpandableTextArea label="Central Theme" value={arch.centralTheme} onChange={(v) => handleChange("centralTheme", v)} placeholder="The one truth this story is telling." rows={3} />
              <ExpandableTextArea label="Opening Image" value={arch.openingImage} onChange={(v) => handleChange("openingImage", v)} placeholder="The first image or moment that sets the world's tone." rows={3} />
              <ExpandableTextArea label="Closing Image" value={arch.closingImage} onChange={(v) => handleChange("closingImage", v)} placeholder="The final image — how has the world changed?" rows={3} />
              <ExpandableTextArea label="Architecture Notes" value={arch.notes} onChange={(v) => handleChange("notes", v)} placeholder="Anything else about how this story is built." rows={3} />
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-6">
            ...
            <div className="rounded-xl p-4 mb-5 text-xs text-slate-600 border"
              style={{ background: `${BRAND.navy}03`, borderColor: `${BRAND.navy}10` }}>
              <span className="font-semibold" style={{ color: BRAND.navy }}>The Dostoevsky Principle.</span>{" "}
              Dostoevsky ended his chapters not with resolution but with pull — a question, a revelation, a moment
              of decision that made stopping impossible. Plan each chapter's ending and its bridge into the next.
              This is where readers are made or lost.
            </div>

            <div className="space-y-3">
              {(arch.chapterPlans || []).map((plan, index) => (
                <ChapterPlanCard key={plan.id} plan={plan} index={index}
                  onChange={(field, value) => updateChapterPlan(index, field, value)}
                  onDelete={() => deleteChapterPlan(index)} />
              ))}
            </div>

            <button onClick={addChapterPlan}
