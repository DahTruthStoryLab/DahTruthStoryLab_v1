...
import { Mic2, Sparkles, RefreshCw } from "lucide-react";
...
const TOPIC_NAMES = VOICE_MODES.map(m => m.label);

export default function VoiceIdentity() {
  const [poem, setPoem, clearPoem]           = usePersistedText("voice-identity-poem", "");
  const [result, setResult, clearResult]     = usePersistedText("voice-identity-result", "");
  const [mode, setMode]                      = useState("analyze");
  const [loading, setLoading]                = useState(false);
  const [showSend, setShowSend]              = useState(false);

  const { prompts, loading: promptsLoading, refresh, updateWithText } = useDynamicPrompts({
    module: "Voice & Identity",
    topics: TOPIC_NAMES,
    genre:  "poetry",
  });
  ...
  return (
    <div className="max-w-2xl">
      <SendToManuscript isOpen={showSend} onClose={() => setShowSend(false)}
        writerText={poem} aiFeedback={result} sourceLabel="Voice & Identity" />
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={refresh} disabled={promptsLoading}
          ...
          style={{ background: "rgba(8,145,178,0.08)", color: BRAND.cyanDark, border: "1px solid rgba(8,145,178,0.2)" }}>
