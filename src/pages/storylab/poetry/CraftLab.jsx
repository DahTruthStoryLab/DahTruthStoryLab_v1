...
const BRAND = { purple: "#4c1d95", purpleLight: "#7c3aed", purplePale: "#ede9fe", gold: "#d4af37", goldDark: "#b8960c" };

const CRAFT_TOPICS = [
  { title: "Line Breaks",         color: "#a78bfa", content: "The line break is the poet's most powerful tool. It controls breath, emphasis, and surprise. Break where meaning pivots — not just where the sentence ends. A line ending on a verb creates tension. A line ending on a preposition pulls the reader forward." },
  { title: "Sound & Music",       color: "#f9a8d4", content: "Poems are heard even when read silently. Listen for assonance (vowel echo), consonance (consonant repetition), and the rhythm of stressed and unstressed syllables. Sound should reinforce meaning — hard consonants for conflict, open vowels for grief or awe." },
  { title: "Metaphor & Image",    color: "#d4af37", content: "The best metaphors are surprising yet inevitable. Avoid dead metaphors. Build images that are concrete and specific — not 'sadness' but 'the drawer I haven't opened since.' Images do emotional work that abstractions cannot." },
  { title: "Tension & Turn",      color: "#67e8f9", content: "Every strong poem has a turn — a volta — where something shifts. It may be a reversal, a revelation, or a deepening. The tension before the turn is what makes the turn land. Ask: where does your poem surprise itself?" },
  { title: "Clarity & Compression", color: "#6ee7b7", content: "Poetry earns its density. Every word must justify its place. Cut adjectives that explain rather than show. Cut lines that summarize what the poem has already enacted. What remains should feel inevitable." },
];
...
export default function CraftLab() {
  ...
  const [poem, setPoem, clearPoem]               = usePersistedText("craft-lab-poem", "");
  const [feedback, setFeedback, clearFeedback]   = usePersistedText("craft-lab-feedback", "");
  ...
  return (
    <div className="max-w-2xl">
      <SendToManuscript isOpen={showSend} onClose={() => setShowSend(false)}
        writerText={poem} aiFeedback={feedback} sourceLabel="Craft Lab" />
            ...
            {open === i && (
              <div className="px-5 pb-5">
                ...
                <div className="rounded-xl px-4 py-3 text-sm italic"
                  ...
                  {prompts[topic.title] || <span className="text-slate-400">{promptsLoading ? "Generating..." : "Loading prompt..."}</span>}
                </div>
              </div>
            )}
          </div>
      ...
      <div className="rounded-3xl p-6"
        ...
        <p className="text-xs text-slate-500 mb-4">Paste your poem for specific, line-level craft feedback. Prompts above will also update to reference your work.</p>
        ...
        <div className="flex items-center flex-wrap gap-3 mt-4">
          ...
          <button onClick={() => setShowSend(true)} disabled={!poem.trim() && !feedback.trim()}
          ...
          {(poem || feedback) && (
            <button onClick={() => { clearPoem(); clearFeedback(); }}
