// ─────────────────────────────────────────────
//  DahTruth StoryLab – AuthorPage.tsx
//  Full author profile hub with edit mode
// ─────────────────────────────────────────────
...
function SectionHeader({ title, sub }: { title: string; sub?: string }) {
  return (
    <div className="mb-6">
      <h2 className="text-2xl font-semibold tracking-wide" style={{ fontFamily: 'EB Garamond, serif', color: '#c9a84c' }}>
        {title}
      </h2>
      {sub && <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.45)' }}>{sub}</p>}
      <div className="mt-2 h-px w-16" style={{ background: 'rgba(201,168,76,0.4)' }} />
    </div>
  );
}

// ─────────────────────────────────────────────
//  EDIT MODAL – Author Profile
// ─────────────────────────────────────────────
function EditProfileModal({
  ...
  onSave: (p: Partial<AuthorProfile>) => void;
  onClose: () => void;
}) {
  const [form, setForm] = useState<AuthorProfile>({ ...profile });
  const [genreInput, setGenreInput] = useState('');
  const [themeInput, setThemeInput] = useState('');
  const fileRef = useRef<HTMLInputElement>(null);

  const field = (key: keyof AuthorProfile) => (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>
  ) => setForm(f => ({ ...f, [key]: e.target.value }));

  const socialField = (key: keyof AuthorProfile['socialLinks']) => (
  ...
  const addTheme = (t: string) => {
    if (t && !form.themes.includes(t)) setForm(f => ({ ...f, themes: [...f.themes, t] }));
    setThemeInput('');
  };
  ...
  const inputCls = `w-full rounded-lg px-4 py-2.5 text-sm outline-none transition-all focus:ring-1`;
  const inputStyle = {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto"
      ...
      <div className="w-full max-w-2xl my-8 rounded-2xl shadow-2xl"
        style={{ background: '#0d1535', border: '1px solid rgba(201,168,76,0.25)' }}>

        {/* Header */}
        <div className="flex items-center justify-between px-8 py-6"
          style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <h2 className="text-xl font-semibold" style={{ fontFamily: 'EB Garamond, serif', color: '#c9a84c' }}>
            Edit Author Profile
          </h2>
          <button onClick={onClose} className="text-zinc-400 hover:text-white transition-colors text-2xl leading-none">&times;</button>
        </div>
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs mt-1.5" style={{ color: 'rgba(255,255,255,0.35)' }}>JPG, PNG — stored locally</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              ['penName', 'Pen Name', 'text'],
            ].map(([key, label]) => (
              <div key={key}>
                <label className="block text-xs mb-1.5 font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>{label}</label>
                <input className={inputCls} style={inputStyle}
                  value={(form as any)[key] || ''} onChange={field(key as keyof AuthorProfile)} />
              </div>
            ))}
          </div>

          <div>
            <label className="block text-xs mb-1.5 font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>Tagline</label>
            <input className={inputCls} style={inputStyle} placeholder="e.g. Writer. Storyteller. Builder of truth."
              value={form.tagline || ''} onChange={field('tagline')} />
          </div>

          <div>
            <label className="block text-xs mb-1.5 font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>Bio</label>
            <textarea className={inputCls} style={inputStyle} rows={4}
              value={form.bio || ''} onChange={field('bio')} />
          </div>

          <div>
            <label className="block text-xs mb-1.5 font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>Writing Mission</label>
            <textarea className={inputCls} style={inputStyle} rows={2}
              value={form.writingMission || ''} onChange={field('writingMission')} />
          </div>

          <div>
            <label className="block text-xs mb-1.5 font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>Personal Quote or Scripture</label>
            <input className={inputCls} style={inputStyle}
              value={form.personalQuote || ''} onChange={field('personalQuote')} />
          </div>

          {/* Genres */}
          <div>
            <label className="block text-xs mb-1.5 font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>Genres</label>
            <div className="flex flex-wrap gap-2 mb-2">
              {form.genres.map(g => (
                <TagPill key={g} label={g} onRemove={() => setForm(f => ({ ...f, genres: f.genres.filter(x => x !== g) }))} />
              ))}
            </div>
            <select className={inputCls} style={inputStyle} value="" onChange={e => addGenre(e.target.value)}>
          ...
          {/* Themes */}
          <div>
            <label className="block text-xs mb-1.5 font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>Themes</label>
            <div className="flex flex-wrap gap-2 mb-2">
              {form.themes.map(t => (
                <TagPill key={t} label={t} onRemove={() => setForm(f => ({ ...f, themes: f.themes.filter(x => x !== t) }))} />
              ))}
            </div>
            <select className={inputCls} style={inputStyle} value="" onChange={e => addTheme(e.target.value)}>
          ...
          {/* Social Links */}
          <div>
            <label className="block text-xs mb-3 font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>Social &amp; Contact Links</label>
            <div className="space-y-3">
              {([
                ...
                <div key={key} className="flex items-center gap-3">
                  <span className="text-xs w-44 flex-shrink-0" style={{ color: 'rgba(255,255,255,0.45)' }}>{label}</span>
                  <input className={`${inputCls} flex-1`} style={inputStyle} placeholder="https://"
                    value={form.socialLinks?.[key] || ''} onChange={socialField(key)} />
                </div>
              ))}
              <div className="flex items-center gap-3">
                <span className="text-xs w-44 flex-shrink-0" style={{ color: 'rgba(255,255,255,0.45)' }}>📧 Contact Email</span>
...
// ─────────────────────────────────────────────
//  BOOK CARD (used in the hub listing)
// ─────────────────────────────────────────────
function BookCard({ book, onEdit }: { book: Book; onEdit: (b: Book) => void }) {
  return (
    <div className="group relative rounded-xl overflow-hidden transition-all hover:scale-[1.02] cursor-pointer"
      ...
      <div className="p-4">
        ...
        {book.genre && <p className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>{book.genre}</p>}
        {book.isFeatured && (
          <span className="inline-block mt-2 text-xs px-2 py-0.5 rounded-full"
            style={{ background: 'rgba(201,168,76,0.15)', color: '#c9a84c' }}>★ Featured</span>
        )}
      </div>
      <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
...
export default function AuthorPage() {
  const {
    ...
    saveProfile, saveBook, deleteBook, createBook, toggleFeatured,
  } = useAuthorProfile();

  const [editingProfile, setEditingProfile] = useState(false);
  const [editingBook, setEditingBook]       = useState<Book | null>(null);
  const [activeTab, setActiveTab]           = useState<'overview' | 'projects' | 'catalog'>('overview');

  const isNew = !profile.fullName;

  // ── Blog feed (static placeholder — connect to DahTruth later) ──
  const blogPosts = [
      ...
      <div className="relative overflow-hidden" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute bottom-0 left-0 w-64 h-64 rounded-full opacity-5"
            style={{ background: 'radial-gradient(circle, #6b4fa0 0%, transparent 70%)', transform: 'translate(-30%, 30%)' }} />
        </div>
        ...
        <div className="relative max-w-5xl mx-auto px-8 py-12">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            <div className="flex-shrink-0">
              <div className="w-32 h-32 rounded-full overflow-hidden flex items-center justify-center text-5xl relative"
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  {profile.bio && (
                    <div>
                      <h3 className="text-xs font-semibold uppercase tracking-widest mb-3"
                        style={{ color: 'rgba(255,255,255,0.35)' }}>Biography</h3>
                      <p className="leading-relaxed text-sm" style={{ color: 'rgba(255,255,255,0.7)', fontFamily: 'EB Garamond, serif', fontSize: '1rem' }}>
                        {profile.bio}
                      </p>
                    </div>
                  )}
                  {profile.writingMission && (
                    <div className="rounded-xl p-6" style={{ background: 'rgba(201,168,76,0.06)', border: '1px solid rgba(201,168,76,0.15)' }}>
                      <h3 className="text-xs font-semibold uppercase tracking-widest mb-3"
                        style={{ color: 'rgba(201,168,76,0.7)' }}>Writing Mission</h3>
...
function BookEditModal({
  ...
  const field = (key: keyof Book) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) =>
    setForm(f => ({ ...f, [key]: e.target.value }));
  ...
  const inputCls = `w-full rounded-lg px-4 py-2.5 text-sm outline-none transition-all focus:ring-1`;
  const inputStyle = { background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', color: '#e8e0d4' };

  const GENRE_LIST = ['Fiction','Urban Fiction','Fantasy','Sci-Fi','Mystery','Thriller','Romance','Historical Fiction',
    'Literary Fiction','Horror','Nonfiction','Memoir','Essay','Poetry','Christian Fiction','Inspirational','Political Commentary'];
      ...
      <div className="w-full max-w-2xl my-8 rounded-2xl shadow-2xl"
        ...
        <div className="px-8 py-6 space-y-5">
          ...
          <div className="flex items-start gap-5">
            <div className="w-24 h-32 rounded-lg overflow-hidden flex-shrink-0 flex items-center justify-center text-3xl"
            ...
            <div className="flex-1">
              ...
              <p className="text-xs mt-1.5" style={{ color: 'rgba(255,255,255,0.3)' }}>JPG, PNG — stored locally</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="date" className={inputCls} style={inputStyle}
                value={form.publishDate || ''} onChange={field('publishDate')} />
            </div>
          </div>
            ...
            <div className="space-y-2.5">
              {([
                ...
                <div key={key} className="flex items-center gap-3">
                  ...
                  <input className={`${inputCls} flex-1`} style={inputStyle} placeholder="https://"
                    value={(form.buyLinks as any)[key] || ''}
